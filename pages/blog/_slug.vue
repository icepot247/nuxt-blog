<template>
  <div>
    <div v-if="social" class="w-20 h-auto share hidden md:flex flex-col items-center justify-between">
      <a :href="`https://www.facebook.com/sharer.php?u=https://nuxt-content-blog-theme.vercel.app/${article.slug}`"
         target="_blank" class="mb-5">
    ...
    <div class="article-head w-full h-auto bg-indigo-200 mb-8">
      <div class="flex flex-col">
        ...
        <div class="flex flex-row items-center justify-center pb-10">
          <img src="https://avatars2.githubusercontent.com/u/34214453" alt="avatar"
               class="rounded-full h-10 w-10 shadow">
          <span class="text-base ml-4 text-gray-700">{{article.author}}</span> <span
          class="text-base ml-4 text-gray-700">|</span>
          <p class="ml-4">{{ formatDate(article.updatedAt) }}</p>
          <br>
        </div>
      </div>
    </div>
        ...
        meta: [
          {
            hid: 'description',
            name: 'description',
            content: (this.article && this.article.description)?this.article.description:''
          },
          {
            hid: 'keywords',
            name: 'keywords',
            content: (this.article && this.article.keywords)?this.article.keywords:''
          },
          {
            hid: 'author',
            name: 'author',
            content: (this.article && this.article.author)?this.article.author:''
          },
          {
            hid: 'robots',
            name: 'robots',
            content: (this.article && this.article.robots)?this.article.robots:''
          }
        ]
      }
    },
